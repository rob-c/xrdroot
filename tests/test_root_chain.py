...
def test_one_path_or_glob_on_its_own_is_one_source_not_a_run_of_letters():
    with chain("tree", str(DATA / "chain.flat.*.root")) as made:
        assert made.files == FLAT
...
def test_trees_this_library_wrote_chain_up_rows_text_and_directories(tmp_path):
    for part in range(3):
        with create(str(tmp_path / f"part{part}.root")) as out:
            rows = Jagged(np.arange(4.0) + 10 * part, [0, 1, 4])
            out["d/events"] = {"x": rows, "s": [f"a{part}", f"b{part}"]}
    with chain("d/events", str(tmp_path / "part*.root")) as made:
        assert len(made) == 6
        assert made["x"].array(1, 3).tolist() == [[1.0, 2.0, 3.0], [10.0]]
        assert made.arrays(["s"], 1, 5, library="pd")["s"].tolist() == ["b0", "a1", "b1", "a2"]
        assert [len(batch["x"]) for batch in made.iterate(step=4)] == [4, 2]
